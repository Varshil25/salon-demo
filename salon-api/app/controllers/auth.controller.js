...
exports.login = async (req, res) => {
  try {
    ...
    if(user.role.role_name == role.BARBER){
      const barberRole = await barber.findOne({ where: { UserId: user.id } });
    ...
    // Generate JWT token
    const token = jwt.sign({ id: user.id, email: user.email, role: user.role.role_name,barberId:barberId ,salonId:salonId}, secretKey, {
      expiresIn: '365d'
    });
    ...
    return sendResponse(res, true, "Login successful", { user: userWithoutPassword, token,salon:salonData,berber:berberData });
