...
exports.getAllUsers = async (req, res) => {
  ...
  try {
    // Fetch roles based on the type: 'user' (Admin, Salon Owner, Barber) or 'customer' (Customer)
    let filter = {};
    ...
    if (type === 'user') {
      const userRoles = await Role.findAll({
        where: { role_name: [ role.SALON_OWNER, role.BARBER] },
        attributes: ['id']
      });
...
exports.findOne = async (req, res) => {
  try {
    const { role: userRole, id: customerId } = req.user; // Extract role and IDs from token
