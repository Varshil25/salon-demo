...
const HaircutDetailsTable: React.FC = () => {
  ...
  const [haircutToDelete, setHaircutToDelete] = useState<number | null>(null);
  ...
  useEffect(() => {
    const fetchHaircutDetails = async () => {
      try {

        const response: any = await axios.get(HAIRCUT_DETAILS_ENDPOINT);
  ...
  return (
    <React.Fragment>
      ...
      <DeleteModal
        ...
        recordId={haircutToDelete?.toString()}
      />
    </React.Fragment>
  );
};
