...
import { fetchHaircutDetails } from "Services/HaircutDetails";
...
const CustomerTable: React.FC = () => {
  ...
  const [selectedCustomerId, setSelectedCustomerId] = useState<number | null>(null); // State for the customer to delete
  ...
  const handleSearchText = (search: any) => {
    ...
    getCustomers(selectedCurrentPage, search);
    // Handle page change logic here
  };
  ...
  const handleDeleteUser = async () => {
    if (selectedCustomerId !== null) {
      try {
        await axios.delete(`${USERS_ENDPOINT}/${selectedCustomerId}`);
        ...
        setSelectedCustomerId(null); // Reset selected user ID
      } catch (error) {
        ...
        cell: (cell: { getValue: () => number; row: { original: any } }) => (
          <div>
            <button
              ...
              onClick={() => handleCardEdit(cell.row.original)}
            >
              Details
            </button>
          </div>
  ...
  const handleCardEdit = async (arg: any) => {
    ...
    setSelectedCutomerDetails(userDetails);
    setModal(true);
  ...
  const onClickDelete = (customer: Customer) => {
    setSelectedCustomerId(customer.id); // Set the selected role ID for deletion
  ...
  return (
    <React.Fragment>
      ...
      <DeleteModal
        ...
        recordId={
          selectedCustomerId !== null ? selectedCustomerId.toString() : undefined
        }
