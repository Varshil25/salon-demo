...
import { tasklist } from "../../../../common/data/kanban";
...
const Board = () => {
  ...
  useEffect(() => {
    const socket = io("https://api.orioniktechnologies.com", {
  ...
  const findTaskById = (id: any) => {
    return tasklist
      .map((project) => project.cards) // Get the tasks arrays
      .flat() // Flatten the array to get all tasks at the top level
      .find((task) => task.id === id); // Find the specific task by ID
  };
  ...
  const handleBarberChange = async (event: any) => {
    if (event.target.value) {
      ...
      if (selectedTotalServiceTime) {
        ...
        appointmentFormik.setFieldValue("barber_id", event.target.value);
        setIsAppointmentAvailable(false);
        toast.warning("Please first select atleast one service!!!");
      }
    }
  ...
  const haircutFormik: any = useFormik({
    ...
    onSubmit: async (values: any) => {
      ...
      toast.success("Haircut details added successfully", { autoClose: 3000 });
                                            ...
                                            <div className="d-flex align-items-start justify-content-between">
                                              <div className="d-block">
                                                ...
                                                <div className="flex-grow-1">
                                                  ...
                                                  <span>
                                                    {card.estimated_wait_time > 60
                                                      ? `${Math.floor(card.estimated_wait_time / 60)} hr ${card.estimated_wait_time % 60
                                                      } min`
      ...
      >
        ...
        <ModalBody>
          ...
          >
            ...
            <div className="form-group mb-3">
              <Label htmlFor="salon" className="col-form-label">
                Salon Name
              </Label>
              <Col lg={12}>
                <Input
            ...
            <div className="form-group mb-3">
              <Label htmlFor="barber" className="col-form-label">
                Barber Name
              </Label>
              <Col lg={12}>
                <Input
