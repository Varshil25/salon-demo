...
const SalonTable: React.FC = () => {
  ...
  const salonId = newSalon?.id ?? 0; // Set a default id if undefined
      ...
      },
      {
        ...
        accessorFn: (row: Salon) => {
          ...
          if (row.weekend_start && row.weekend_end) {
            ...
            return `${formatTime(row.open_time)} - ${formatTime(
              row.close_time
            )}`; // Fallback if working hours are not set
          }
        },
        enableColumnFilter: false,
      },
  ...
  const onClickDelete = (salon: any) => {
    ...
    setSelectedSalonId(salon.id); // Ensure this is correctly assigning the ID
    setDeleteModal(true);
  };

  const handleCloseModal = () => {
    setDeleteModal(false); // Close the modal
    setSelectedSalonId(null); // Clear selected barber ID
  };
  ...
  const handleDeleteSalon = async () => {
    ...
    try {
      if (selectedSalonId !== null) {
        console.log("Deleting salon with ID:", selectedSalonId); // Debugging log
        await deleteSalon(selectedSalonId);
  ...
  const handleSearchText = (search: any) => {
    ...
    fetchSalonsList(selectedCurrentPage, search);
    // Handle page change logic here
  };
  ...
  const handleUpdateSalon = async (id: number, updatedSalonData: any) => {
    try {
      ...
      toast.error("Failed to update salon", { autoClose: 3000 });
    }
  };
  ...
  // All possible closing times
  const allClosingTimes = [
    "12:00 PM",
    "12:30 PM",
    "1:00 PM",
    "1:30 PM",
  ...
  // Helper function to convert 12-hour time (AM/PM) to 24-hour format
  ...
  // Function to generate closing time options based on the opening time
  const getClosingTimeOptions = (openTime: string) => {
    const openTimeInMinutes = convertTo24HourFormat(openTime);

    // Filter closing times to only those that are after the selected open time
    return allClosingTimes.filter((closeTime) => {
      const closeTimeInMinutes = convertTo24HourFormat(closeTime);
  ...
  return (
    <React.Fragment>
                    ...
                    <div className="avatar-lg">
                      <div className="avatar-title bg-light rounded-circle">
                        ...
                        {Array.isArray(selectedImages) &&
                          ...
                          <div>No image selected</div> // Optional: Display message if no image is selected
                        )}
                      </div>
                    </div>
                  </div>
      ...
      <DeleteModal
        ...
        recordId={selectedSalonId != null ? String(selectedSalonId) : undefined} // Convert to string if not undefined
      />
