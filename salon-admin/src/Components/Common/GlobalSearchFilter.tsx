            ...
            <Col xxl={3} sm={4}>
                <div className="input-light">
                    <select className="form-control" data-choices data-choices-search-false name="choices-single-default" id="idStatus">
                        ...
                        <option value="Open">Check IN</option>
                        <option value="Inprogress">In Salon</option>
...
interface TaskListGlobalFilterProps {
    ...
    initialDateRange?: Date[]; // Optional prop for initial date range
    initialStatus?: string;    // Optional prop for initial status
}

const TaskListGlobalFilter: React.FC<TaskListGlobalFilterProps> = ({ onFilterSubmit,
    initialDateRange = [new Date()], // Default to today if no initial value provided
    initialStatus = "all" 
 }) => {
    const [dateRange, setDateRange] = useState<Date[] | null>(null); // State for date range
    const [status, setStatus] = useState<string>("all"); // State for status
    
    // Set initial values from props when the component mounts
    useEffect(() => {
        setDateRange(initialDateRange);
        setStatus(initialStatus);
    }, [initialDateRange, initialStatus]);
    // Handle Filters button click
    const handleFiltersClick = () => {
        if (onFilterSubmit) {
            onFilterSubmit({ dateRange, status }); // Pass both date range and status
        }
    };
    return (
        <React.Fragment>
            <div className="col-xxl-3 col-sm-3">
                <Flatpickr
                    ...
                    onChange={(selectedDates: any) => setDateRange(selectedDates)} // Update date range state
                />
            </div>
